...
nonum.table <- function(dafra){
  ...
  tb1 <- lapply(data.frame(dafra[,sapply(dafra,is.logical)]),table) #draw the table for logical variables
  tb2 <- lapply(data.frame(dafra[,sapply(dafra,is.factor)]),table) #draw the table for factor variables
  return(list(tb1,tb2))
}
...
rsquare <- function(num){
  ...
  if (ncol(num) <= 1){
    RR ='sorry we cannot count pairwise R spuare because of the number of numeric variables'
  }else{
...
Cor_pearson <- function(num,threshold){
  ...
  if (ncol(num) <= 1){
    CRR ='sorry we cannot count pairwise correlation coefficient because of the number of numeric variables'
  }else{
  colna <- colnames(num) # take out all the variables' names
  com_num <- combn(colna, 2) #combine the names pairwise
  VP2 <- c()
...
plot_categ_binary <- function(data_frame,switch){
  ...
  if(ncol(data_frame)==0){
    return("there is neither factor nor logical nor binary variables")
  }else{
